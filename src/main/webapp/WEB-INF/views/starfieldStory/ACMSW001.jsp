...
<div id="blog-mng" class="contents-area" data-bcn_cd="${bcn_cd}">
	...
	<div class="contents-wrap">
		<h1 class="menu-title">스타필드 스토리 관리 - 블로그 관리 - 블로그 목록</h1>
		<div class="list-top">
		...
		<table id="blog-mng-list" class="basic-list">
			...
			<thead>
				<tr>
					...
					<th {{#if(hashObject.sort_name == 'blog_titl')}}class="selected"{{/if}} on-click="sort:blog_titl">타이틀</th>
			...
			<tbody>
				...
				<tr>
					<td>{{rnum}}</td>
					<td on-click="detail:{{blog_seq}}" class="ellipsis ta-l cursor-p">{{blog_titl}}</td>
					<td class="bg-img-cell">
						<span class="img-wrap"><img src="{{img_uri}}"></span>
					</td>
					<td class="preview-cell">
						<img class="icon-preview" src="/images/icon_preview.png" alt="미리보기 이미지" on-click="link:{{blog_url}}">
					...
					<td>{{reg_dttm}}</td>
					<td><input name="groupCheckBox" type="checkbox" value="{{blog_seq}}"></td>
				</tr>
				{{/list}}
			</tbody>
		</table>
...
<script src="/js/app/starfieldStory/ACMSW001.js" type="text/javascript"></script>
