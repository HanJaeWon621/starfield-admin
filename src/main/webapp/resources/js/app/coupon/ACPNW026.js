...
//대사조정처리
var compareEqualCp = function(){
	var log_seq = $("#log_seq").val();
	//console.log(log_seq);
	//return;
	if(log_seq==''){
		alert("대사 조정할 목록을 선택하세요.");
		return;
	}
	if(!confirm("대사 조정을 하시겠습니까?")){
		return false;
	}else{
		//대사 조정 처리
		var restUri = window.location.origin + '/rest/01/compareEqaulCp/'+log_seq;
...
var getUseCoupons = function(offset, limit) {
	...
	var offset = offset || 1;
	var limit = limit || 10;
	var restUri = window.location.origin + '/rest/01/getUseCompareCoupons';
	restUri += '?';
